...
class Chunk$1 {
    ...
    render(options, addons, outputChunk) {
        ...
        for (let i = 0; i < this.dependencies.length; i++) {
            ...
            let relPath = this.id ? normalize(path.relative(path.dirname(this.id), depId)) : depId;
            ...
            for (const { node, resolution } of module.dynamicImports) {
                ...
                if (resolution instanceof Module) {
                    if (resolution.chunk !== this && isChunkRendered(resolution.chunk)) {
                        ...
                        let relPath = normalize(path.relative(path.dirname(this.id), resolutionChunk.id));
                        if (!relPath.startsWith('../'))
                            relPath = './' + relPath;
                    ...
                    if (resolution.renormalizeRenderPath) {
                        resolutionId = normalize(path.relative(path.dirname(this.id), resolution.renderPath));
